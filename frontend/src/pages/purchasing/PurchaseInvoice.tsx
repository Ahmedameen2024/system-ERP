...
export default function PurchaseInvoice() {
  const [invoices, setInvoices] = useState([
    { id: '1', code: 'PINV-0045', supplier: 'شركة البيان للمقاولات والتجارة', date: '١٤/٠٧/٢٠٢٥', amount: 23400, status: 'Posted' },
    { id: '2', code: 'PINV-0044', supplier: 'مؤسسة الرياض للتوريدات الكهربائية', date: '١٢/٠٧/٢٠٢٥', amount: 87900, status: 'Approved' },
    { id: '3', code: 'PINV-0043', supplier: 'المصنع السعودي الموحد', date: '١٠/٠٧/٢٠٢٥', amount: 120000, status: 'Draft' }
  ]);

  const [showModal, setShowModal] = useState(false);
  const [lines, setLines] = useState<InvoiceLine[]>([
    { itemName: 'شاشة سامسونج LED 55 بوصة', qty: 10, cost: 2000, taxPct: 15 }
  ]);
  ...
  const addLine = () => {
    setLines([...lines, { itemName: '', qty: 1, cost: 0, taxPct: 15 }]);
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    const code = 'PINV-' + String(invoices.length + 1).padStart(4, '0');
    setInvoices([...invoices, { id: Date.now().toString(), code, supplier: form.supplier, date: '١٤/٠٧/٢٠٢٥', amount: grandTotal, status: form.status }]);
    setShowModal(false);
  };
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <table className="data-table">
          ...
          <tbody>
            {invoices.map(inv => (
              <tr key={inv.id}>
                <td><span className="numeric" style={{ fontWeight: 700, color: 'var(--color-primary)' }}>{inv.code}</span></td>
                <td style={{ fontWeight: 500 }}>{inv.supplier}</td>
                <td className="numeric">{inv.date}</td>
                <td className="numeric" style={{ textAlign: 'left', fontWeight: 700 }}>{inv.amount.toLocaleString('ar-SA')} ر.س</td>
                <td>
                  <span className={`chip ${
                    inv.status === 'Posted' ? 'chip-success' :
                    inv.status === 'Approved' ? 'chip-info' : 'chip-neutral'
                  }`}>
                    {inv.status === 'Posted' ? 'مرحلة ومقيدة' :
                     inv.status === 'Approved' ? 'معتمدة' : 'مسودة'}
                  </span>
                </td>
                <td>
                  <button className="btn btn-ghost btn-sm" onClick={() => alert('تمت محاكاة ترحيل الفاتورة للقيد المزدوج بنجاح')} title="ترحيل مباشر" disabled={inv.status === 'Posted'}>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-box" style={{ maxWidth: '800px' }} onClick={e => e.stopPropagation()}>
            <h2 style={{ fontSize: '1.125rem', fontWeight: 700, marginBottom: '1.5rem' }}>إنشاء فاتورة مشتريات جديدة</h2>
