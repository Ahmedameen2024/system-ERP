...
exports.createCurrencyTransfer = exports.getCurrencyTransfers = exports.deleteBankAccount = exports.updateBankAccount = exports.createBankAccount = exports.getBankAccountById = exports.getBankAccounts = exports.deleteCashBox = exports.updateCashBox = exports.createCashBox = exports.getCashBoxById = exports.getCashBoxes = void 0;
...
const createCashBox = async (req, res) => {
    try {
        ...
        const { code, nameAr, nameEn, branchId, currencyId, currencyIds, currencies, glAccountId, responsibleEmployeeId, openingBalance = 0, maximumBalance = 0, status = 'Active', notes, } = req.body;
        ...
        if (!code || !nameAr || !branchId || chosenCurrencyIds.length === 0 || !glAccountId) {
            (0, response_1.errorResponse)(res, 'جميع الحقول الأساسية مطلوبة (كود الصندوق، الاسم العربي، الفرع، العملات، الحساب المحاسبي)', 400);
            return;
        }
      ...
      RETURNING *`, [
        ...
        if (currencies && Array.isArray(currencies) && currencies.length > 0) {
            for (let i = 0; i < currencies.length; i++) {
                ...
                await (0, db_1.query)(`INSERT INTO cash_box_currencies (cash_box_id, currency_id, opening_balance, current_balance, maximum_balance, is_default)
           VALUES ($1, $2, $3, $3, $4, $5)
           ON CONFLICT (cash_box_id, currency_id) DO NOTHING`, [createdBox.id, cId, openBal, maxBal, i === 0]);
            }
        }
        else {
            for (let i = 0; i < chosenCurrencyIds.length; i++) {
                ...
                await (0, db_1.query)(`INSERT INTO cash_box_currencies (cash_box_id, currency_id, opening_balance, current_balance, maximum_balance, is_default)
           VALUES ($1, $2, $3, $3, $4, $5)
           ON CONFLICT (cash_box_id, currency_id) DO NOTHING`, [createdBox.id, cId, openBal, Number(maximumBalance) || 0, i === 0]);
            }
        }
...
const updateCashBox = async (req, res) => {
    try {
        ...
        const companyId = req.user.companyId;
        const userId = req.user.userId;
        const { code, nameAr, nameEn, branchId, currencyId, currencyIds, currencies, glAccountId, responsibleEmployeeId, openingBalance, maximumBalance, status, notes, } = req.body;
       ...
       RETURNING *`, [
        ...
        if (currencies && Array.isArray(currencies) && currencies.length > 0) {
            for (let i = 0; i < currencies.length; i++) {
                ...
                await (0, db_1.query)(`INSERT INTO cash_box_currencies (cash_box_id, currency_id, opening_balance, current_balance, maximum_balance, is_default)
           VALUES ($1, $2, 0, 0, $3, $4)
           ON CONFLICT (cash_box_id, currency_id) DO UPDATE SET is_default = $4, maximum_balance = $3`, [id, cId, maxBal, i === 0]);
            }
        }
            ...
            for (let i = 0; i < chosenCurrencyIds.length; i++) {
                ...
                await (0, db_1.query)(`INSERT INTO cash_box_currencies (cash_box_id, currency_id, opening_balance, current_balance, maximum_balance, is_default)
           VALUES ($1, $2, 0, 0, $3, $4)
           ON CONFLICT (cash_box_id, currency_id) DO UPDATE SET is_default = $4`, [id, cId, Number(maximumBalance) || 0, i === 0]);
            }
            await (0, db_1.query)(`DELETE FROM cash_box_currencies WHERE cash_box_id = $1 AND currency_id != ALL($2) AND current_balance = 0`, [id, chosenCurrencyIds]);
        }
...
const createBankAccount = async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.userId;
        const { code, nameAr, nameEn, branchId, currencyId, currencyIds, glAccountId, accountNumber, iban, swift, contactPerson, phone, email, openingBalance = 0, status = 'Active', notes, } = req.body;
        const chosenCurrencyIds = currencyIds && Array.isArray(currencyIds) && currencyIds.length > 0
            ? currencyIds
            : (currencyId ? [currencyId] : []);
      ...
      RETURNING *`, [
        ...
        for (let i = 0; i < chosenCurrencyIds.length; i++) {
            const cId = chosenCurrencyIds[i];
            const openBal = (i === 0 && openingBalance !== undefined) ? Number(openingBalance) || 0 : 0;
            await (0, db_1.query)(`INSERT INTO bank_account_currencies (bank_account_id, currency_id, opening_balance, current_balance, is_default)
         VALUES ($1, $2, $3, $3, $4)
         ON CONFLICT (bank_account_id, currency_id) DO NOTHING`, [createdBank.id, cId, openBal, i === 0]);
        }
...
const updateBankAccount = async (req, res) => {
    try {
        ...
        const companyId = req.user.companyId;
        const userId = req.user.userId;
        const { code, nameAr, nameEn, branchId, currencyId, currencyIds, glAccountId, accountNumber, iban, swift, contactPerson, phone, email, openingBalance, status, notes, } = req.body;
       ...
       RETURNING *`, [
        ...
        if (chosenCurrencyIds.length > 0) {
            for (let i = 0; i < chosenCurrencyIds.length; i++) {
                ...
                await (0, db_1.query)(`INSERT INTO bank_account_currencies (bank_account_id, currency_id, opening_balance, current_balance, is_default)
           VALUES ($1, $2, 0, 0, $3)
           ON CONFLICT (bank_account_id, currency_id) DO UPDATE SET is_default = $3`, [id, cId, i === 0]);
            }
            await (0, db_1.query)(`DELETE FROM bank_account_currencies WHERE bank_account_id = $1 AND currency_id != ALL($2) AND current_balance = 0`, [id, chosenCurrencyIds]);
        }
...
// ==========================================
const getCurrencyTransfers = async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const result = await (0, db_1.query)(`SELECT ct.*,
       ...
       ORDER BY ct.transfer_date DESC, ct.created_at DESC`, [companyId]);
        (0, response_1.successResponse)(res, result.rows);
    }
    catch (error) {
        (0, response_1.errorResponse)(res, 'خطأ في جلب عمليات تحويل وصرف العملة', 500, error.message);
    }
};
exports.getCurrencyTransfers = getCurrencyTransfers;
const createCurrencyTransfer = async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.userId;
        const branchId = req.user.branchId;
        const { transferDate, sourceCashBoxId, sourceBankAccountId, sourceCurrencyId, sourceAmount, targetCashBoxId, targetBankAccountId, targetCurrencyId, targetAmount, exchangeRate, differenceAmount = 0, notes, } = req.body;
        if (!transferDate || !sourceCurrencyId || !targetCurrencyId || Number(sourceAmount) <= 0 || Number(targetAmount) <= 0 || Number(exchangeRate) <= 0) {
            (0, response_1.errorResponse)(res, 'تاريخ التحويل والعملات والمبالغ وسعر الصرف مطلوبة وقيمها أكبر من صفر', 400);
            return;
        }
        if (!sourceCashBoxId && !sourceBankAccountId) {
            (0, response_1.errorResponse)(res, 'يجب تحديد الصندوق أو الحساب البنكي المصدر', 400);
            return;
        }
        if (!targetCashBoxId && !targetBankAccountId) {
            (0, response_1.errorResponse)(res, 'يجب تحديد الصندوق أو الحساب البنكي المستلم', 400);
            return;
        }
        await (0, db_1.transaction)(async (client) => {
            // 1. Verify available balance in source currency
            if (sourceCashBoxId) {
                const balRes = await client.query(`SELECT current_balance FROM cash_box_currencies WHERE cash_box_id = $1 AND currency_id = $2 FOR UPDATE`, [sourceCashBoxId, sourceCurrencyId]);
                ...
                await client.query(`UPDATE cash_box_currencies SET current_balance = current_balance - $1 WHERE cash_box_id = $2 AND currency_id = $3`, [sourceAmount, sourceCashBoxId, sourceCurrencyId]);
            }
            else if (sourceBankAccountId) {
                const balRes = await client.query(`SELECT current_balance FROM bank_account_currencies WHERE bank_account_id = $1 AND currency_id = $2 FOR UPDATE`, [sourceBankAccountId, sourceCurrencyId]);
                const currBal = balRes.rows.length > 0 ? Number(balRes.rows[0].current_balance) : 0;
                if (currBal < Number(sourceAmount)) {
                    const curRes = await client.query(`SELECT code FROM currencies WHERE id = $1`, [sourceCurrencyId]);
                    const curCode = curRes.rows[0]?.code || '';
                ...
                await client.query(`UPDATE bank_account_currencies SET current_balance = current_balance - $1 WHERE bank_account_id = $2 AND currency_id = $3`, [sourceAmount, sourceBankAccountId, sourceCurrencyId]);
            }
            // 2. Add to target cash box / bank account in target currency
            if (targetCashBoxId) {
                await client.query(`INSERT INTO cash_box_currencies (cash_box_id, currency_id, current_balance, opening_balance, is_default)
           VALUES ($1, $2, $3, 0, FALSE)
           ON CONFLICT (cash_box_id, currency_id)
           DO UPDATE SET current_balance = cash_box_currencies.current_balance + $3`, [targetCashBoxId, targetCurrencyId, targetAmount]);
            }
            else if (targetBankAccountId) {
                await client.query(`INSERT INTO bank_account_currencies (bank_account_id, currency_id, current_balance, opening_balance, is_default)
           VALUES ($1, $2, $3, 0, FALSE)
           ON CONFLICT (bank_account_id, currency_id)
           DO UPDATE SET current_balance = bank_account_currencies.current_balance + $3`, [targetBankAccountId, targetCurrencyId, targetAmount]);
            }
            // 3. Generate Double-entry Journal Entry
            const transferNumber = `FX-${Date.now()}`;
            const jeNumber = `JE-FX-${Date.now()}`;
            // Get GL accounts
            let sourceGlAccountId = null;
            let targetGlAccountId = null;
            if (sourceCashBoxId) {
          ...
          currency_id, exchange_rate, total_debit, total_credit, created_by, status, posted_at
            ...
            if (targetGlAccountId) {
                await client.query(`INSERT INTO journal_entry_lines (journal_entry_id, gl_account_id, cash_box_id, bank_account_id, debit, credit, debit_base, credit_base, line_description, sort_order)
           ...
           VALUES ($1,$2,$3,$4,0,$5,0,$6,$7,1)`, [
            ...
            // 4. Record Transfer
            const transferRes = await client.query(`INSERT INTO currency_transfers (
        ...
        RETURNING *`, [
            ...
            (0, response_1.successResponse)(res, transferRes.rows[0], 'تم تنفيذ عملية تحويل وصرف العملة بنجاح وتحديث الأرصدة والقيود المحاسبية', 201);
        });
    }
    catch (error) {
        (0, response_1.errorResponse)(res, error.message || 'خطأ في تنفيذ عملية تحويل وصرف العملة', 500);
    }
};
exports.createCurrencyTransfer = createCurrencyTransfer;
